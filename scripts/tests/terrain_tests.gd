...
func _test_noise_image() -> void:
	var image: Image = Image.create(Generator.GRID_WIDTH, Generator.GRID_HEIGHT, false, Image.FORMAT_RGBA8)
	...
	var result: Variant = Generator.new().generate(image)
...
func _test_single_black_bar() -> void:
	...
	var bar_width := 43
	var bar_start := int((Generator.GRID_WIDTH - bar_width) / 2)
	for y in range(Generator.GRID_HEIGHT):
	...
	for y in range(0, Generator.GRID_HEIGHT, 16):
		var material := _material_at(result, Vector2i(bar_start + int(bar_width / 2), y))
	...
	_assert(_material_at(result, Vector2i(2, int(Generator.GRID_HEIGHT / 2))) == Generator.AIR, "Weisser Bereich links vom Balken ist nicht frei")
	_assert(_material_at(result, Vector2i(Generator.GRID_WIDTH - 3, int(Generator.GRID_HEIGHT / 2))) == Generator.AIR, "Weisser Bereich rechts vom Balken ist nicht frei")
